...
mu2 = sig2 / (sig2-sig);
if sig < 0
    checkcvg = @(mu,res,errtol) ...
        ( (mu > mu1) | ((abs(mu) > errtol) & (mu < mu2)) ) ...
           & ((abs(sig)*res./((mu-1).^2))<errtol );
else  % sig > 0
    checkcvg = @(mu,res,errtol) ...
        ( ((abs(mu) > errtol) & (mu < mu1)) | (mu > mu2) ) ...
          & ((abs(sig)*res./((mu-1).^2))<errtol );
end
...
[mu,X,ncg,iter] = LanFRO(applyC,applyM,v,tol,checkcvg,nev,maxit);
