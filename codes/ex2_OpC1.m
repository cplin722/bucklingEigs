%
% Script files: ex2_OpC1.m
%
% This is a demo script for example2.
% The matrix-vector product u=Cv is computed by
...
mu2 = sig2 / (sig2-sig);
if sig < 0
    findCvg = @(mu,res) ( (mu > mu1) | ((abs(mu) > tol) & (mu < mu2)) ) ...
                          & ((abs(sig)*res./((mu-1).^2))<tol);
else  % sig > 0
    findCvg = @(mu,res) ( ((abs(mu) > tol) & (mu < mu1)) | (mu > mu2) ) ...
                          & ((abs(sig)*res./((mu-1).^2))<tol);
end
...
t0_lan=tic;
[mu,X,ncg,iter] = LanFRO(applyC,applyM,findCvg,v,nev,maxit);
time_lan=toc(t0_lan);
